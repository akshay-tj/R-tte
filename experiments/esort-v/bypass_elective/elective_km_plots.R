...
#   - elective_cohort   : cohort tibble with columns STUDY_ID,
#                             NvrEpisode.ProcedureStartDate, early_surgery
#   - mortality_clean       : tibble with columns study_id, death_date
#   - elective_outcomes : tibble with columns study_id, afs_days,
...
mortality_km_plot_path <- "Z:/PHP/HSR/ESORT-V/ESORT-V/bypass_elective_270426/mortality_km_plot.png"
afs_km_plot_path       <- "Z:/PHP/HSR/ESORT-V/ESORT-V/bypass_elective_270426/afs_km_plot.png"

# =============================================================================
# KM curve — Mortality
# =============================================================================
df_mortality <- elective_cohort %>%
...
# =============================================================================
df_afs <- elective_cohort %>%
  select(STUDY_ID, early_surgery) %>%
  mutate(study_id = as.character(STUDY_ID)) %>%
  left_join(
    elective_outcomes %>% select(study_id, afs_days, afs_event),
    by = "study_id"
  ) %>%
  filter(afs_days >= 0)
